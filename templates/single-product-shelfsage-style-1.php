...
?>
                        ...
                        <?php if (filter_var($settings['enable_look_inside'] ?? true, FILTER_VALIDATE_BOOLEAN) && $look_inside_url): ?>
                        ...
                        <div class="mt-auto">
                            <div class="flex flex-col sm:flex-row gap-4 mb-6">
                                ...
                                <div class="flex-1 flex flex-wrap gap-4">
                                    <?php $hide_cart = filter_var($settings['hide_add_to_cart'] ?? false, FILTER_VALIDATE_BOOLEAN); ?>
                                    <?php $show_cart = !$hide_cart; ?>
                                        ...
                                        <form class="cart flex-1" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
                                            ...
                                            <div class="flex flex-col gap-4">
                                                ...
                                                <div class="flex flex-wrap gap-4">
                                                    <button type="submit" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>" class="flex-1 bg-gray-900 hover:bg-black text-white font-bold py-2.5 px-6 rounded-full shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 flex justify-center items-center gap-2 single_add_to_cart_button button alt text-sm">
                                    ...
                                    <?php if (filter_var($settings['enable_custom_button'] ?? false, FILTER_VALIDATE_BOOLEAN)): ?>
                                        <?php
                                        $custom_btn_url = (!empty($affiliates) && !empty($affiliates[0]['url'])) ? $affiliates[0]['url'] : '#product-details';
                                        ?>
                                        <a href="<?php echo esc_url($custom_btn_url); ?>" target="<?php echo (strpos($custom_btn_url, '#') === 0) ? '_self' : '_blank'; ?>" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 border-2 border-gray-900 hover:bg-gray-900 hover:text-white text-gray-900 font-bold rounded-full transition-all">
                                            <?php echo esc_html(rmss_get_label('custom_button', 'View Details', $labels)); ?>
                                        </a>
                                    <?php endif; ?>
                                    <?php if (!$product->is_in_stock()): ?>
                                        <p class="text-red-500 font-bold bg-red-50 p-3 rounded-lg text-center">Out of Stock</p>
                                    <?php endif; ?>
                                </div>
                            ...
                            <?php if (!empty($affiliates) && filter_var($settings['enable_affiliate'] ?? true, FILTER_VALIDATE_BOOLEAN)): ?>
...
endforeach; ?>
        ...
        <!-- ============================================ -->
        <!-- Customer Reviews Section (Style 1 — Blue)   -->
        <!-- ============================================ -->
...
<!-- Look Inside Modal (style-aware, reads pdf_reader_style from settings) -->
...
// Modal logic is handled by includes/look-inside-modal.php (style-aware)
</script>
...
/* ===== Scoped WooCommerce Review Styles — Style 1 (Blue) ===== */
...
.rmss-reviews-style-1 .commentlist .star-rating { color: #f59e0b; font-size: 0.9rem; margin-bottom: 0.5rem; }
...
.rmss-reviews-style-1 .comment-form #submit { background: #2563eb; color: #fff; font-weight: 700; padding: 0.65rem 1.75rem; border-radius: 9999px; border: none; cursor: pointer; transition: background 0.2s; font-size: 0.9rem; }
