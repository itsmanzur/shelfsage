...
$release_date = get_post_meta($product_id, '_rmss_release_date', true);
$ebook_url = get_post_meta($product_id, '_rmss_ebook_url', true);
$audio_url = get_post_meta($product_id, '_rmss_audio_url', true);
...
$is_preorder = !empty($pre_order) && !empty($release_date);
$release_ts = $is_preorder ? strtotime($release_date) : 0;
$release_in_future = $release_ts > 0 && $release_ts > time();
...
if ($avg_rating > 0 && apply_filters('rmss_show_product_rating', true, $product_id)):
    echo '<div class="rmss-product-rating flex items-center gap-2 mt-1">';
    echo wc_get_rating_html($avg_rating, $review_count);
    if ($review_count > 0) {
        echo '<span class="text-xs text-gray-500">(' . absint($review_count) . ' ' . esc_html(_n('review', 'reviews', $review_count, 'shelfsage')) . ')</span>';
    }
    echo '</div>';
endif;
?>
                        ...
                        <div class="mt-auto">
                            <div class="flex flex-col sm:flex-row gap-4 mb-6">
                                ...
                                <div class="flex-1 flex flex-wrap gap-4">
                                    <?php $show_cart = !filter_var($settings['hide_add_to_cart'] ?? false, FILTER_VALIDATE_BOOLEAN) && $product->is_in_stock(); ?>
                                    <?php if ($show_cart): ?>
                                            ...
                                            <div class="flex flex-col gap-4">
                                                ...
                                                <div class="flex flex-wrap gap-4">
                                                    <button type="submit" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>" class="flex-1 bg-gray-900 hover:bg-black text-white font-bold py-2.5 px-6 rounded-full shadow-lg hover:shadow-xl transition-all flex justify-center items-center gap-2 single_add_to_cart_button button alt text-sm">
                                                        ...
                                                        <?php echo $is_preorder && $release_in_future ? esc_html(apply_filters('rmss_preorder_button_text', __('Pre-order', 'shelfsage'), $product_id)) : esc_html(rmss_get_label('add_to_cart', 'Add to Cart', $labels)); ?>
                                                    </button>
                                                </div>
                                                <?php if ($is_preorder && $release_in_future && $release_date): ?><p class="text-xs text-gray-500 mt-1"><?php echo esc_html(apply_filters('rmss_preorder_release_text', sprintf(__('Releases on %s', 'shelfsage'), date_i18n(get_option('date_format'), $release_ts)), $release_ts, $product_id)); ?></p><?php endif; ?>
                                            </div>
...
endforeach; ?>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 rmss-animate-slide-up" style="animation-delay: 400ms">
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-blue-600 font-bold uppercase tracking-wider"><?php esc_html_e( 'Author', 'shelfsage' ); ?></span>
                            </div>
                        </div>
        ...
        <!-- Related Books (same author / genre / publisher) -->
...
?>
        <div class="max-w-7xl mx-auto px-4 mt-8 pb-16">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 rmss-animate-slide-up">
                <h2 class="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3 border-l-4 border-blue-500 pl-4">
...
/* ===== Scoped WooCommerce Review Styles — Default (Blue) ===== */
.rmss-reviews-style-1 #reviews { margin: 0; padding: 0; }
.rmss-reviews-style-1 #comments h2 { font-size: 1.1rem; font-weight: 700; color: #111827; margin-bottom: 1.5rem; }
.rmss-reviews-style-1 .commentlist { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 1.25rem; }
.rmss-reviews-style-1 .commentlist li.review { background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 1rem; padding: 1.25rem 1.5rem; }
.rmss-reviews-style-1 .commentlist .star-rating { color: #f59e0b; margin-bottom: 0.5rem; }
.rmss-reviews-style-1 .commentlist .reviewer { font-weight: 700; color: #1e40af; font-size: 0.95rem; }
.rmss-reviews-style-1 .commentlist time { font-size: 0.75rem; color: #9ca3af; margin-left: 0.5rem; }
.rmss-reviews-style-1 .commentlist .description p { color: #4b5563; font-size: 0.95rem; line-height: 1.6; margin-top: 0.5rem; }
.rmss-reviews-style-1 #review_form_wrapper { margin-top: 2rem; border-top: 1px solid #e5e7eb; padding-top: 2rem; }
.rmss-reviews-style-1 #reply-title { font-size: 1.1rem; font-weight: 700; color: #111827; margin-bottom: 1rem; }
.rmss-reviews-style-1 .comment-form p label { display: block; font-size: 0.85rem; font-weight: 600; color: #374151; margin-bottom: 0.35rem; }
.rmss-reviews-style-1 .comment-form input[type="text"],
.rmss-reviews-style-1 .comment-form input[type="email"],
.rmss-reviews-style-1 .comment-form textarea { width: 100%; padding: 0.6rem 0.9rem; border: 1px solid #d1d5db; border-radius: 0.5rem; font-size: 0.95rem; outline: none; transition: border-color 0.2s; }
.rmss-reviews-style-1 .comment-form input:focus,
.rmss-reviews-style-1 .comment-form textarea:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.1); }
.rmss-reviews-style-1 .comment-form #submit { background: #2563eb; color: #fff; font-weight: 700; padding: 0.65rem 1.75rem; border-radius: 9999px; border: none; cursor: pointer; transition: background 0.2s; }
.rmss-reviews-style-1 .comment-form #submit:hover { background: #1d4ed8; }
.rmss-reviews-style-1 .stars span a { color: #f59e0b !important; }
/* Upsells */
...
.rmss-upsells-wrap .price { color: #2563eb; font-weight: 700; font-size: 0.9rem; }
</style>
