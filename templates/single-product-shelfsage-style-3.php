...
<div class="rmss-wrapper w-full bg-gray-50">
    ...
    <div class="rmss-container min-h-screen pb-20 font-sans">
        ...
        <div class="shelfsage-single-product shelfsage-pro-layout-style-3 py-16 bg-[#0f172a] text-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                <span class="inline-block py-1 px-3 rounded bg-blue-900/50 text-blue-300 text-xs font-bold uppercase tracking-widest mb-2 border border-blue-800">Split View 3D Layout</span>
            </div>
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-700">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 mb-6">
                                ...
                                <div class="flex-1 flex flex-wrap gap-4">
                                    ...
                                    <?php if (filter_var($settings['enable_custom_button'] ?? false, FILTER_VALIDATE_BOOLEAN)): ?>
                                        ...
                                        <a href="<?php echo esc_url($custom_btn_url); ?>" target="<?php echo (strpos($custom_btn_url, '#') === 0) ? '_self' : '_blank'; ?>" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 border-2 border-white hover:bg-white hover:text-[#0f172a] text-white font-bold rounded-full transition-all"><?php echo esc_html(rmss_get_label('custom_button', 'View Details', $labels)); ?></a>
                                    <?php endif; ?>
...
endforeach; ?>
        ...
        <!-- ================================================== -->
        <!-- Customer Reviews Section (Style 3 — Slate/Dark)    -->
        <!-- ================================================== -->
        <?php if (wc_reviews_enabled() && (comments_open() || $product->get_review_count() > 0)): ?>
        <div class="max-w-7xl mx-auto px-4 mt-12 mb-4">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 rmss-animate-slide-up">
                <h2 class="text-2xl font-bold text-gray-900 mb-8 flex items-center gap-3 border-l-4 border-gray-800 pl-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                    <?php esc_html_e('Customer Reviews', 'shelfsage'); ?>
                    <?php if ($product->get_review_count() > 0): ?>
                    <span class="text-sm font-normal text-gray-600 bg-gray-100 rounded-full px-3 py-1">
                        <?php echo esc_html($product->get_review_count()); ?> <?php echo esc_html(_n('review', 'reviews', $product->get_review_count(), 'shelfsage')); ?>
                    </span>
                    <?php
    endif; ?>
                </h2>
                <div class="rmss-reviews-wrap rmss-reviews-style-3">
        ...
        <!-- ================================================== -->
        <!-- WooCommerce Upsells (Style 3 — Dark)               -->
        <!-- ================================================== -->
        <?php
...
?>
        <div class="max-w-7xl mx-auto px-4 mt-8 pb-16">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 rmss-animate-slide-up">
                <h2 class="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3 border-l-4 border-gray-800 pl-4">
...
/* ===== Scoped WooCommerce Review Styles — Style 3 (Slate/Dark) ===== */
.rmss-reviews-style-3 #reviews { margin: 0; padding: 0; }
.rmss-reviews-style-3 #comments h2 { font-size: 1.1rem; font-weight: 700; color: #111827; margin-bottom: 1.5rem; }
.rmss-reviews-style-3 .commentlist { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 1.25rem; }
.rmss-reviews-style-3 .commentlist li.review { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 1rem; padding: 1.25rem 1.5rem; border-left: 3px solid #334155; }
.rmss-reviews-style-3 .commentlist .star-rating { color: #f59e0b; font-size: 0.9rem; margin-bottom: 0.5rem; }
.rmss-reviews-style-3 .commentlist .reviewer { font-weight: 700; color: #1e293b; font-size: 0.95rem; }
.rmss-reviews-style-3 .commentlist time { font-size: 0.75rem; color: #9ca3af; margin-left: 0.5rem; }
.rmss-reviews-style-3 .commentlist .description p { color: #4b5563; font-size: 0.95rem; line-height: 1.6; margin-top: 0.5rem; }
.rmss-reviews-style-3 #review_form_wrapper { margin-top: 2rem; border-top: 1px solid #e2e8f0; padding-top: 2rem; }
.rmss-reviews-style-3 #reply-title { font-size: 1.1rem; font-weight: 700; color: #111827; margin-bottom: 1rem; }
.rmss-reviews-style-3 .comment-form p label { display: block; font-size: 0.85rem; font-weight: 600; color: #374151; margin-bottom: 0.35rem; }
.rmss-reviews-style-3 .comment-form input[type="text"],
.rmss-reviews-style-3 .comment-form input[type="email"],
.rmss-reviews-style-3 .comment-form textarea { width: 100%; padding: 0.6rem 0.9rem; border: 1px solid #d1d5db; border-radius: 0.5rem; font-size: 0.95rem; outline: none; transition: border-color 0.2s; }
.rmss-reviews-style-3 .comment-form input:focus,
.rmss-reviews-style-3 .comment-form textarea:focus { border-color: #334155; box-shadow: 0 0 0 3px rgba(51,65,85,0.1); }
.rmss-reviews-style-3 .comment-form #submit { background: #1e293b; color: #fff; font-weight: 700; padding: 0.65rem 1.75rem; border-radius: 9999px; border: none; cursor: pointer; transition: background 0.2s; font-size: 0.9rem; }
.rmss-reviews-style-3 .comment-form #submit:hover { background: #0f172a; }
.rmss-reviews-style-3 .stars span a { color: #f59e0b !important; }
/* Upsells */
...
.rmss-upsells-wrap .price { color: #1e293b; font-weight: 700; font-size: 0.9rem; }
</style>
